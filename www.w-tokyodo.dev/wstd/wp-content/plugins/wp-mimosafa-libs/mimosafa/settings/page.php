...
class Page {
	...
	public function __construct( $page = null, $page_title = null, $menu_title = null ) {
		...
		if ( $page && is_array( $page ) ) {
			$this->init_array( $page );
		} else {
			$this->init( $page, $page_title, $menu_title );
		}
	}

	private function init_array( Array $args, $n = -1 ) {
		static $hierarchy = ++$n;
		//
	}
	...
	public function callback( $callback, $sanitize = null, $args = [] ) {
		if ( is_string( $callback ) && method_exists( __CLASS__, 'callback_' . $callback ) ) {
			$callable = [ &$this, 'callback_' . $callback ];
